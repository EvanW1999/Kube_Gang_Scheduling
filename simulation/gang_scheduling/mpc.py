...
from simulation.config.config import (
    ...
    GANG_SCHEDULING_CHECKPOINT_PENALTY,
...
from simulation.gang_scheduling.resource_configurer import (
    ResourceConfigurer,
    ConfigurationWindow,
)
...
class MPController(ABC):
    ...
    def calculate_time_horizon(
        self, time_step: int, current_config: Dict[str, int]
    ) -> int:
        pass
...
class DynamicMPController(MPController):
    ...
    def calculate_duration(self, configuration_window: ConfigurationWindow) -> float:
        resource_configuration: Dict[str, int] = (
            self.resource_configurer.calculate_resource_configurations(
                configuration_window=configuration_window
            )
        )
        return self.resource_configurer.calculate_estimated_runtime(
    ...
    def calculate_time_horizon_from_start(self, time_step: int, start: int) -> None:
        ...
        for window_size in range(
            ...
            min(
                ...
                self.simulation_length - time_step,
                FORECASTER_WINDOW_SIZE - 5,
            )
            + 1,
        ):
            configuration_window: ConfigurationWindow = ConfigurationWindow(
                ...
                window_size=window_size,
                starting_prediction=start,
            )
            end: int = window_size + start
            additional_duration: float = (
                self.dp_durations[end] + GANG_SCHEDULING_CHECKPOINT_PENALTY
                if end < self.window_size
                else 0
            )
            duration: float = self.calculate_duration(configuration_window) * pow(
                self.length_punishment, window_size
            )

            window_durations[window_size] = duration + additional_duration
        min_window: int = min(
            window_durations, key=window_durations.get
        )  # type: ignore
        self.dp_durations[start] = window_durations[min_window]
        self.dp_window_sizes[start] = min_window

    def calculate_time_horizon_for_current_config(
        self, time_step: int, current_config: Dict[str, int]
    ) -> None:
        min_duration = float("inf")
        for config_keep_length in range(
            1, min(self.window_size, self.simulation_length - time_step) + 1
        ):
            ...
            additional_duration: float = (
                self.dp_durations[config_keep_length]
                + GANG_SCHEDULING_CHECKPOINT_PENALTY
                if config_keep_length < GANG_SCHEDULING_WINDOW_SIZE
                else 0
            )
            min_duration = min(min_duration, config_duration + additional_duration)
    ...
    def calculate_time_horizon(
        ...
        for start in reversed(
            range(min(self.window_size, self.simulation_length - time_step))
        ):
            self.calculate_time_horizon_from_start(time_step=time_step, start=start)
        if time_step != 0:
            self.dp_durations[0] += GANG_SCHEDULING_CHECKPOINT_PENALTY
        if len(current_config) != 0:
            self.calculate_time_horizon_for_current_config(time_step, current_config)
...
class ReinforcementMPController(MPController):
    ...
    def calculate_time_horizon(
        ...
        observation: numpy.ndarray = self.env.get_state_from_time_step(time_step)
