...
DQN_PATH: str = f"{MODEL_PATH}/DQN_MlpPolicy_4_6_scaled"
...
class SimulatorEnv(Env):
    def __init__(
        ...
        self.observation_space: Space = Box(
            low=0, high=1, shape=(num_actions, window_size), dtype="float64"
        )
        self.state = self.get_state_from_time_step(self.time_step)
        self.simulation_resource_configurer = ResourceConfigurer(
            workloads=self.workloads, predictions=actual_workload_sizes
        )
    ...
    def get_durations_from_action(self, time_step: int, action: int) -> numpy.ndarray:
        ...
        if action == 0:
            ...
            config = self.resource_configurer.calculate_resource_configurations(
        ...
        for step in range(self.window_size):
            config_window = ConfigurationWindow(
                simulation_time_step=time_step, window_size=1, starting_prediction=step
            )
            durations[step] = (
                (
                    self.resource_configurer.calculate_estimated_runtime(
    ...
    def step(self, action) -> Tuple[numpy.ndarray, float, bool, Dict[Any, Any]]:
        ...
        next_timestep = ConfigurationWindow(
            simulation_time_step=self.time_step, window_size=1
        )

        if action != 0:
            reward -= self.checkpoint_penalty
            configuration_window = ConfigurationWindow(
                simulation_time_step=self.time_step, window_size=action
            )
            self.current_config = (
