...
from simulation.config.config import (
    GANG_SCHEDULING_MAX_SHARES,
    GANG_SCHEDULING_SHARE_INCREMENT,
    GANG_SCHEDULING_STARTING_SHARES,
    GANG_SCHEDULING_TOTAL_SHARES,
    PROFILER_MAX_SHARES,
    PROFILER_MIN_SHARES,
    PROFILER_OUTPUT_PATH,
    PROFILER_SHARE_INCREMENT,
    SIMULATION_DIR,
    SIMULATION_MAX_WORKLOAD,
    SIMULATION_MIN_WORKLOAD,
    SIMULATION_WORKLOAD_INCREMENT,
)
...
class ResourceConfigurer:
    def __init__(
        self, workloads: List[Workload], predictions: Dict[str, numpy.ndarray]
    ) -> None:
        self.profiling_df: pandas.Dataframe = pandas.read_csv(
            SIMULATION_DIR + PROFILER_OUTPUT_PATH,
            dtype={0: str, 1: "float64", 2: "float64", 3: "float64"},
        )
        self.workloads: List[Workload] = workloads
        self.workload_models: Dict[str, interpolate.RectBivariateSpline] = (
            self.create_workload_models()
        )
    ...
    def create_workload_models(self) -> Dict[str, interpolate.RectBivariateSpline]:
        ...
        for workload in self.workloads:
            workload_values: numpy.ndarray = (
                self.profiling_df[
                    self.profiling_df[self.profiling_df.columns[0]]
                    == workload.task.task_name
                ]
                .sort_values(
                    by=[self.profiling_df.columns[1], self.profiling_df.columns[2]]
                )
                .values
            )
            ...
            workload_models[workload.task.task_name] = interpolate.RectBivariateSpline(
                workloads, cpu_shares, durations
            )
    ...
    def get_slowest_job(
        ...
        for workload in self.workloads:
            ...
            workload_size: int = self.predictions[workload.task.task_name][
                configuration_window.simulation_time_step
            ][configuration_window.starting_prediction + forecast_step]
            cpu_shares: int = resource_configuration[workload.task.task_name]

            duration: float = workload_model(workload_size, cpu_shares, grid="False")[
                0
            ][0]
            results.append(
                RunResult(
    ...
    def find_largest_improvement(self, slowest_jobs: List[RunResult]) -> str:
        ...
        for count, job in enumerate(slowest_jobs):
            ...
            improvement: float = pow(self.delta, count) * (job.runtime - new_runtime)
    ...
    def calculate_resource_configurations(
        self, configuration_window: ConfigurationWindow
    ) -> Dict[str, int]:
        resource_configuration: Dict[str, int] = {
            workload.task.task_name: GANG_SCHEDULING_STARTING_SHARES
            for workload in self.workloads
        }

        for _ in range(
            GANG_SCHEDULING_STARTING_SHARES * len(self.workloads),
            GANG_SCHEDULING_TOTAL_SHARES,
            GANG_SCHEDULING_SHARE_INCREMENT,
        ):
    ...
    def increment_static_configuration(
        self, resource_configuration: Dict[str, int]
    ) -> None:
        workload: Workload
        workload_size: int = int(
            statistics.mean([SIMULATION_MIN_WORKLOAD, SIMULATION_MAX_WORKLOAD])
        )
        ...
        for workload in self.workloads:
            ...
            if cpu_shares < GANG_SCHEDULING_MAX_SHARES:
                job_runtimes.append(
                    RunResult(
                        workload=workload,
                        runtime=workload_model(workload_size, cpu_shares),
                        workload_size=workload_size,
                        cpu_shares=cpu_shares,
                    )
                )
        slowest_result: RunResult = max(job_runtimes, key=attrgetter("runtime"))
        resource_configuration[
            slowest_result.workload.task.task_name
        ] += GANG_SCHEDULING_SHARE_INCREMENT
    ...
    def calculate_estimated_runtime(
        ...
        forecast_step: int
        for forecast_step in range(configuration_window.window_size):
            slowest_job: RunResult = self.get_slowest_job(
                resource_configuration=resource_configuration,
                configuration_window=configuration_window,
...
def main():
    ...
    resource_configurer: ResourceConfigurer = ResourceConfigurer(WORKLOADS, actual)
    print(resource_configurer.workload_models["atomic"](30, 3500, grid="False"))
    print(resource_configurer.workload_models["atomic"](30, 3600, grid="False"))
    print(resource_configurer.workload_models["atomic"](32, 3700, grid="False"))
    print(resource_configurer.workload_models["atomic"](33, 3500, grid="False"))
