...
from simulation.gang_scheduling.mpc import (
    DynamicMPController,
    MPController,
    StaticMPController,
)
...
class Simulator(ABC):
    ...
    def __init__(
        ...
        if self.real_simulation:
            ...
            self.zk_queues: Dict[str, LockingQueue] = {
                workload.task.task_name: LockingQueue(
                    self.zk, f"{workload.task.task_name}"
                )
    ...
    def create_workloads_from_configuration(
        self, configuration: Dict[str, int]
    ) -> None:
        workload: Workload
        for workload in self.workloads:
            kube_create_stress_job(
                env_vars=get_env_vars(
                    task=workload.task, num_tasks=len(self.workloads)
                ),
                cpu_shares=configuration[workload.task.task_name],
            )
    ...
    def simulate_timestep(self, time_step: int) -> float:
        ...
        print(f"Simulation timestep {time_step} has finished with duration {duration}")
        return duration
    ...
    def fake_simulate_timestep(self, time_step: int) -> float:
        return self.fake_resource_configurer.calculate_estimated_runtime(
            self.current_config,
            ConfigurationWindow(
                simulation_time_step=time_step, window_size=1, starting_prediction=0
            ),
        )
...
class StaticSimulator(Simulator):
    ...
    def simulate(self) -> float:
        time_step: int
        self.current_config: Dict[str, int] = (
            self.resource_configurer.calculate_static_configuration()
        )
        if self.real_simulation:
        ...
        for time_step in range(GANG_SCHEDULING_SIMULATION_LENGTH):
            duration: float = (
                ...
                if self.real_simulation
                else self.fake_simulate_timestep(time_step)
            )
            # print(duration)
...
class MPCSimulator(Simulator):
    ...
    def create_new_configuration_from_window(
        self, time_step: int, window_size: int
    ) -> None:
    ...
    def simulate(self) -> float:
        ...
        for time_step in range(GANG_SCHEDULING_SIMULATION_LENGTH):
            ...
            if window_size != 0:
                self.create_new_configuration_from_window(time_step, window_size)
...
def generate_onetime_simulation() -> None:
    ...
    dynamic_mpc: DynamicMPController = DynamicMPController(
        ...
        window_size=GANG_SCHEDULING_SIMULATION_LENGTH,
    )
    dynamic_mpc.calculate_time_horizon(time_step=0)
    get_duration_from_plan(resource_configurer, dynamic_mpc.dp_window_sizes)


def get_duration_from_plan(
    resource_configurer: ResourceConfigurer, mpc_plan: numpy.ndarray
) -> None:
    cur_plan: int = 0
    checkpoints: int = -1
    duration: float = 0
